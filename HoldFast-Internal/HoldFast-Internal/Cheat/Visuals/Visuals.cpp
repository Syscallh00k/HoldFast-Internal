...
void HoldFast::Visuals::DrawVisuals(HoldFast::ClientRoundPlayer round_player) {
	...
	if (bounds.onscreen) {
		...
		if (HoldFast::Features::Visuals::corner_box) {
			float lw = 6.0f;
			float lh = 6.0f;
			ImVec2 tl(bounds.left - 1, bounds.top - 1);
			ImVec2 tr(bounds.right + 1, bounds.top - 1);
			ImVec2 bl(bounds.left - 1, bounds.bottom + 1);
			ImVec2 br(bounds.right + 1, bounds.bottom + 1);
			draw_list->AddLine(tl, ImVec2(tl.x + lw, tl.y), IM_COL32(0, 100, 255, 255));
			draw_list->AddLine(tl, ImVec2(tl.x, tl.y + lh), IM_COL32(0, 100, 255, 255));
			draw_list->AddLine(tr, ImVec2(tr.x - lw, tr.y), IM_COL32(0, 100, 255, 255));
			draw_list->AddLine(tr, ImVec2(tr.x, tr.y + lh), IM_COL32(0, 100, 255, 255));
			draw_list->AddLine(bl, ImVec2(bl.x + lw, bl.y), IM_COL32(0, 100, 255, 255));
			draw_list->AddLine(bl, ImVec2(bl.x, bl.y - lh), IM_COL32(0, 100, 255, 255));
			draw_list->AddLine(br, ImVec2(br.x - lw, br.y), IM_COL32(0, 100, 255, 255));
			draw_list->AddLine(br, ImVec2(br.x, br.y - lh), IM_COL32(0, 100, 255, 255));
		}
		...
		if (HoldFast::Features::Visuals::skeleton) {
			...
			Vector3 spine_pos = player_bones.GetHead().GetPosition(); // Still using this if not in Biped
			spine_pos.y -= .1f;
			if (
